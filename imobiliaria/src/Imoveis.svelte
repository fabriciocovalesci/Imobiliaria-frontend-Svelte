...
async function fetchData() {
    const res = await fetch(`${URLimoveis}`,{
      method: 'GET',
      headers: {
         'Accept': "application/json",
      "Content-Type": "application/json",
...
<main class='top-10 m-10 p-10'>
  ...
  <div class="flex flex-wrap m-5 mx-auto">
   ...
   {:then items}
     {#each items as item}
          <div class="w-full overflow-hidden p-2 m-2 p-2 m-2 mx-auto xl:my-2 xl:px-2 xl:w-1/3">
        <Cards description={item.description}, title={item.title}, photo={item.photo}/>
      </div>
      {/each}
    {:catch error}
