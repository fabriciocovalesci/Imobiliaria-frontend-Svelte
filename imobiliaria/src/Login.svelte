...
<div class="min-h-screen flex items-center  justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
  <div class="max-w-md w-full space-y-8 border border-gray-600 border-solid p-2 rounded-md shadow-2xl">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="text-sm">
          <Router>
          <a href="/register" class="font-medium text-indigo-600 hover:text-indigo-500" use:links Register>
            Crie uma conta
          </a>
        </Router>
        </div>
      ...
      <div>
        <button on:click={()=>{$user=true&&login()}} type="submit" class="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
